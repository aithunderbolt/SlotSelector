  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const renderClassText = (classItem, isFirst) => {
        ...
        // Class Summary (may contain Arabic — use splitTextToSize for wrapping)
        ensureSpace(lineH);
        pdf.setFont('helvetica', 'bold');
      ...
      // This resolves natural sizes + compresses all at once instead of serially per-image.
      const IMG_GAP = 3;
      const IMG_GAP_V = 4;
      const COLS = 2;
      ...
      const allImageEntries = []; // { classIdx, data, fmt, w, h }
      const imageTaskList = [];
      ...
      for (let ci = 0; ci < classDataWithAttachments.length; ci++) {
        ...
        for (let ai = 0; ai < attachments.length; ai++) {
          ...
          imageTaskList.push({ classIdx: ci, attachIdx: ai, dataUri: attachment.data });
        }
      }
      ...
      const imgTasks = imageTaskList.map((item) => async () => {
        // First pass: get natural dimensions to compute target size
        ...
        const key = `${item.classIdx}_${item.attachIdx}`;
        processedImages[key] = { data: compressed, fmt: 'JPEG', w: imgW, h: imgH };
      });
      ...
      for (let i = 0; i < classDataWithAttachments.length; i++) {
        ...
        // Render text block directly via jsPDF (no html2canvas)
