...
const Reports = ({ isSuperAdmin = false }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const addCanvasToPdf = (canvas) => {
        ...
        // How many canvas pixels fit in the available page height
        ...
        while (srcY < canvas.height) {
          ...
          // Start a new page if this slice won't fit
          if (yPosition + sliceMm > pdfHeight - margin) {
            pdf.addPage();
            yPosition = margin;
          }

          // Draw the slice into a temporary canvas and export it
      ...
      for (let i = 0; i < classDataWithAttachments.length; i++) {
        ...
        // Place the text block, slicing across pages if needed
        addCanvasToPdf(textCanvas);

        // --- Step 2: add each attendance image directly to the PDF ---
        // Images are placed 2-per-row. Each image is individually checked for
        // page overflow, so no image is ever clipped or dropped.
        if (classItem.attachments && classItem.attachments.length > 0) {
          const IMG_GAP = 3;    // mm horizontal gap between images in a row
          const IMG_GAP_V = 4;  // mm vertical gap between rows
          const COLS = 2;
          const slotW = (contentWidth - IMG_GAP * (COLS - 1)) / COLS; // mm per slot
          const MAX_H_MM = 90;  // max image height in mm
          ...
          const flushRow = () => {
            ...
            // Start a new page if the row doesn't fit
          ...
          for (const attachment of classItem.attachments) {
            ...
            // Scale to fit within the column slot while preserving aspect ratio
            const scale = Math.min(slotW / natW, MAX_H_MM / natH);
            const imgW = natW * scale;
            const imgH = natH * scale;

            // Compress: re-draw at display pixel size → JPEG 0.72 (big size reduction)
          ...
          flushRow(); // flush the final partial row
        }
      ...
      // Save the PDF
      pdf.save(`Tilawah-Report-${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
  ...
  const generateWord = async () => {
    ...
    try {
      ...
      saveAs(blob, `Tilawah-Report-${new Date().toISOString().split('T')[0]}.docx`);
    } catch (err) {
